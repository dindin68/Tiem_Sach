...
<div class="container mx-auto p-4">
    <div class="flex justify-between p-6 mt-2 bg-orange-50 dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        ...
        <div class="flex flex-row mt-4">
            ...
            <a href="{{ route('admin.books.create') }}"
                class="inline-block bg-green-800 mx-2 text-white px-4 py-2 rounded">Thêm Sách Mới</a>
        </div>
    </div>
    ...
    <form action="{{ route('admin.promotions.apply') }}" method="POST">
        ...
        <div class="mt-4 flex items-center">
            <label for="promotion_id" class="mr-2 font-medium">Chọn khuyến mãi:</label>
            <select name="promotion_id" id="promotion_id" class="border px-3 py-2 rounded">
                @foreach ($promotions as $promotion)
                    <option value="{{ $promotion->id }}">{{ $promotion->name }} - {{ $promotion->di }}%</option>
                @endforeach
            </select>
            <button type="submit" class="bg-blue-700 text-white px-4 py-2 ml-3 rounded">Áp dụng khuyến mãi</button>
        </div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="border p-2">
                        ...
                        <button type="button" class="text-red-500 hover:underline ml-2"
                            onclick="submitDelete('{{ route('admin.books.destroy', $book) }}')">Xóa</button>
                    </td>
                </tr>
                @endforeach
            </tbody>
